    ...
    def graph(self, filename, options, fileprefix=None, graph_variables: List[Run] = None, title=False, series=None):
        ...
        for i, (testie, build, all_results) in enumerate(series):
            for x in all_results:
                ...
                x_vars=pd.concat([pd.DataFrame({'build' :build.pretty_name()},index=[0]), x_vars],axis=1)
        ...
        if not filename:
            pandas_df_name="pandas_" + datetime.datetime.now().strftime("%y%m%d%H%M") +".csv"
        else:
            pandas_df_name=filename.split(".")[0] +"-pandas_" + ".csv"
        all_results_df.to_csv(pandas_df_name, index=True, index_label="test_number", sep=",", header=True)
